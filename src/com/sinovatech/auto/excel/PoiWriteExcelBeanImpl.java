...
import com.sinovatech.auto.util.StringUtil;
...
// 百分数
        cellPercentStyle.setDataFormat((short) 9);
                    ...
                    for (int j = 0; j < map.size(); j++) {
                        ...
                        if (null != strList) {
                            if (strList.contains(j)) {
                                formatCellStr(cell, getValue(map.get(j)), cellPercentStyleStr);
                            } else {
                                formatCell(cell, getValue(map.get(j)), cellPercentStyle);
                            }
                        } else {
                            // 赋值
                            formatCell(cell, getValue(map.get(j)), cellPercentStyle);
                        ...
                        /**    <pre>	赋值
                         // formatCell(cell, getValue(map.get(j)),
                         // cellPercentStyle);
...
// 百分数
        ...
        for (int i = 0; i < dataList.size(); i++) {
            ...
            for (int j = 0; j < dataMap.size(); j++) {
                cell = row.createCell((short) j);
                // 赋值
                formatCell(cell, getValue(dataMap.get(j)), cellPercentStyle);
...
// 百分数
        ...
        for (int i = 0, n = dataList.size(); i < n; i++) {
            ...
            for (int j = 0, m = dataMap.size(); j < m; j++) {
                cell = row.createCell((short) j);
                /**<pre> cell.setCellValue(getValue(dataMap.get(j)));
                 // 赋值</pre>*/
                formatCell(cell, getValue(dataMap.get(j)), cellPercentStyle);
...
// 创建一行，存放表头 row =
            ...
            for (int cm = 0; contentList != null && cm < contentList.size(); cm++) {
                ...
                for (int j = 0; j < contentMap.size(); j++) {
                    cell = row.createCell((short) j);
                    // 赋值
                    formatCell(cell, getValue(contentMap.get(j)), cellPercentStyle);
...
// 颜色list
        List<Integer> colorList = em.getTitleColor();
...
// 得到背景色
        List<Integer> bgcolorList = em.getTitleBgColor();
     ...
     * public void writeExcelHead(List titleList,Workbook wb, Sheet sheet,int
     // 声明样式对象和设置字体 CellStyle style =
     * cellNum) throws Exception {

     // 创建报表头部 Cell cell; for(int t =0;t <
     * this.setStyle(wb);
    ...
    private void writeUniteHead(List fialList, ExcelModel em, Workbook wb, Sheet sheet, List uniteRowList)
        ...
        for (int u = 0; u < uniteRowList.size(); u++) {
            Map uniteRowMap = new HashMap(16);
        ...
        row.setHeightInPoints(em.getTitleH());
// 得到背景色
        List<Integer> colorList = em.getTitleBgColor();
    ...
    public Workbook JudgeExcelType(String excelName) throws Exception {
        ...
        try {
            ...
            if ("xls".equals(excelType) || "xlsx".equals(excelType)) {
    ...
    public CellStyle setTitleRead(Workbook wb) {
        ...
        /**<pre>设定样式的的背景颜色
         ...
         // style.setFillPattern(HSSFCellStyle.SOLID_FOREGROUND);</pre> */
        style.setFont(font);
        return style;
    }
    ...
    public CellStyle setTitleBgColor(Workbook wb, String str, boolean bb) {
        ...
        if (bb) {
             ...
             *  字号
             // font.setFontHeightInPoints((short) 10);
             // 粗体
             // font.setBoldweight(HSSFFont.BOLDWEIGHT_BOLD);</pre>*/
            style.setFont(font);
        }
...
// 如果为数字型
        if (StringUtil.isNumberAll(value)) {
...
// 去掉%后 如果全为数字，则转成百分数格式
                if (StringUtil.isNumber(strTemp)) {
